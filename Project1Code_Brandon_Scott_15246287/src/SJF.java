...
public class SJF {
	...
	public static void main(String args[]) {
       Task T6 = new Task("T6", 0, 0);//placeholder Task object used to swap values.
	  ...
	  for(int k =0; k < taskArray.length; k++) {  
	    for (int j=k+1; j < taskArray.length; j++) { //this for loop organizes the list from shortest to longest job, and passes that 
	    	                                  //info into
	    	if(taskArray[k].getBurst() >taskArray[j].getBurst()) {      //swap elements if not in order
                T6.setBurst(taskArray[k].getBurst());     //double for loop used to swap values to prepare to print shortest job first
                taskArray[k].setBurst(taskArray[j].getBurst());    
                taskArray[j].setBurst(T6.getBurst()); 
                T6.setPriority(taskArray[k].getPriority());     
                taskArray[k].setPriority(taskArray[j].getPriority());    
                taskArray[j].setPriority(T6.getPriority()); 
                T6.setName(taskArray[k].getName());     
                taskArray[k].setName(taskArray[j].getName());    
                taskArray[j].setName(T6.getName());
                
              }     	
	    	}
	    }
	    
		System.out.println("SJF:"); //Print out the name of the scheduling algorithm
